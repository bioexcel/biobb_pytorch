...
class TestAllModels:
    ...
    @pytest.mark.parametrize(
        "model_type,extra_props,expected_attrs", [
            ('AutoEncoder', {}, [
                'encoder', 'decoder', 'norm_in']), ('VariationalAutoEncoder', {}, [
                    'encoder', 'decoder', 'mean_nn', 'log_var_nn']), ])
    def test_build_all_model_types(
        ...
        try:
            ...
            ), f"Model file should exist for {model_type}"
    ...
    @pytest.mark.parametrize(
        "model_type,custom_layers", [
            ('AutoEncoder', {
                'encoder_layers': [
                    32, 16, 8], 'decoder_layers': [
                    8, 16, 32]}), ('VariationalAutoEncoder', {
                        'encoder_layers': [
                            32, 16], 'decoder_layers': [
                                16, 32]}), ])
